...
async def on_message(message):
    ...
    if "potato" in message.content.lower():
        await message.channel.send('potato!!!')    #return 'potato'
...
@bot.command(brief='Register for the money game.')
async def register(ctx):
    ...
    if user_id not in data:
        ...
        data[user_id] = new_playerData
        save_data(data)
        
        balance = int(data[user_id]["balance"])
        energy = data[user_id]["energy"]
        user = await bot.fetch_user(user_id)
        await ctx.send(f"Hello {user}, please do {prefix}help for more info!")
    else:
...
async def lb(ctx):
    ...
    tempBalBoard = {}
    leaderboard = []
    for user_id in data.keys():    #there we go
        user = await bot.fetch_user(user_id)
        tempBalBoard[user] = data[user_id]['balance']
        
    sortedBalBoard = sorted(tempBalBoard.items(), key=lambda x: x[1], reverse=True)
    #print leaderboard 
    leaderboard = "\n".join([f"{user.name}: ${balance}" for user, balance in sortedBalBoard])  #??? CHATGPT :(
...
async def w(ctx, place: str):
    ...
    if place == "j":
        ...
        try:
            randWork = random.randint((6, 10) +  data[user_id]['base'])  #rand num
...
####TOKEN! KEEP SAFE!
bot.run('NjY4NTQxNTUzNzk0NDgyMTk1.Gh-QVO.vahnS0HNPebJBsv8Kk6nHbYQUFlcucpVw1RrMY')
